  ...
  const handleClick = (id) =>{
    ...
    return (
      <div className='grid justify-center items-center m-8 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
        {doctors.map((doctor) => (
         <button className='hover:shadow-xl shadow-blue-400' onClick={()=> handleClick(doctor.id)}><DoctorCardAppointment id={doctor.id} image={doctor.photo} name={doctor.name} rating={doctor.avgRating} address={doctor.hospital}/></button>
        ))}
      </div>
    );
  };
  
  export default AppointyList;